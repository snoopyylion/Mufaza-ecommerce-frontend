...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      let orderData = {
        address: formData, // delivery address from form
        items: orderItems, // products in the cart
        amount: getCartAmount() + delivery_fee, // total amount
        customerName: `${formData.firstName} ${formData.lastName}`, // combine first and last name
        customerEmail: formData.email, // customer email
        customerPhone: formData.phone // customer phone number
      };
      ...
      switch(method){
        ...
        case 'cod':
          ...
          default:
            ...
            case 'flutterwave':
              const flwResponse = await axios.post(`${backendUrl}/api/order/flutterwave`, orderData, { headers: { token } });
              if (flwResponse.data.success) {
                  window.location.href = flwResponse.data.payment_link; // Redirect to Flutterwave payment
              } else {
                  toast.error(flwResponse.data.message);
              }
              break;
      }
